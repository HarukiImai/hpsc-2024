...
void smooth_data(double* data, int nx, int ny) {
    for (int j = 1; j < ny - 1; ++j) {
        for (int i = 1; i < nx - 1; ++i) {
            data[j * nx + i] = (data[j * nx + i] + data[j * nx + (i - 1)] + data[j * nx + (i + 1)] + 
                                data[(j - 1) * nx + i] + data[(j + 1) * nx + i]) / 5.0;
        }
    }
}
...
int main (){
    ...
    for(n=0; n<nt; n++){
        ...
        if (n % 100 == 0){
            ...
            smooth_data(v, nx, ny);
            smooth_data(p, nx, ny);
            printf("Smoothing data at step %d\n", n);
        }
